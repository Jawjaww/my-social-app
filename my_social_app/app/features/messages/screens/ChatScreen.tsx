  ...
  useEffect(() => {
    const loadMessages = async () => {
      try {
        ...
        const messagesFromDb = await getMessages(contactUid);
        console.log('Messages retrieved from database:', messagesFromDb);
        // Convert dates to timestamps for Redux
        const serializedMessages = messagesFromDb.map(msg => ({
          ...msg,
          createdAt: msg.createdAt instanceof Date ? msg.createdAt.getTime() : msg.createdAt
        }));
        dispatch(setMessages({ channelId: contactUid, messages: serializedMessages }));
      } catch (error) {
